...
vector<string> TheLastFour::simulateAnOverChase(const vector<Player>& players)
{
	// Generate uniformly distributed random number
	random_device rd;
	mt19937 gen(rd());
	...
	// Display initial match summary
	if (win) {
		cout << "Enchai wins !" << endl << endl;
	}
	else {
		cout << "Lengaburu wins !" << endl << endl;
	}
...
vector<string> TheLastFour::simulateAnOverSetTarget(const vector<Player>& players)
{
	// Generate uniformly distributed random number
	random_device rd;
	mt19937 gen(rd());
